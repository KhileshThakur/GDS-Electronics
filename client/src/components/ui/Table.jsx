...
// Static map, not a template literal — Tailwind's scanner needs to see the
// full class strings written literally in the source to generate their CSS.
// Building "hidden md:table-cell" via `hidden ${x}:table-cell` gets silently
// purged: only "hidden" survives, so the column disappears at every width.
...
const hideClass = (hideBelow) => HIDE_BELOW_CLASSES[hideBelow] || "";
...
// A column's `id` is its identity for the Columns menu / persisted order+visibility.
// Its `key` stays reserved for data access (getNestedValue) and sorting — keeping
// these separate means a dotted data path like "category.name" can never collide
// with the menu's own state.
const colId = (column) => column.id || column.key;
...
const ColumnsMenu = ({ columns, order, hidden, onToggle, onMove }) => {
    ...
    const byId = Object.fromEntries(columns.map((c) => [colId(c), c]));
    ...
    return (
        <div className="relative" ref={ref}>
            ...
            {open && (
                <div className="absolute right-0 z-20 mt-1 w-56 border border-[var(--border)] bg-[var(--surface)] py-1 shadow-lg">
                    {order.map((id, index) => {
                        ...
                        return (
                            <div key={id} className="flex items-center gap-2 px-3 py-1.5 text-xs hover:bg-[var(--background)]">
                                <input
                                    type="checkbox"
                                    checked={!isHidden}
...
const Table = ({
    ...
    const columnIds = useMemo(() => columns.map(colId), [columns]);
    ...
    const [order, setOrder] = useState(() => loadPersisted()?.order || columnIds);
    const [hidden, setHidden] = useState(
        () => new Set(loadPersisted()?.hidden || columns.filter((c) => c.defaultHidden).map(colId))
    );

    useEffect(() => {
        setOrder((current) => {
            const known = current.filter((id) => columnIds.includes(id));
    ...
    useEffect(() => {
        ...
        try {
            localStorage.setItem(`table-cols:${persistKey}`, JSON.stringify({ order, hidden: [...hidden] }));
        } catch {
    ...
    const byId = useMemo(() => Object.fromEntries(columns.map((c) => [colId(c), c])), [columns]);
    const visibleColumns = order.filter((id) => !hidden.has(id) && byId[id]).map((id) => byId[id]);
    ...
    const filteredData = useMemo(() => {
        ...
        // Only search columns currently visible — hide a column in the Columns
        // menu and it's excluded from search too, so the two stay in sync.
        const searchScope = visibleColumns.length ? visibleColumns : columns;

        return sortedData.filter((row) =>
            searchScope.some((column) => {
                const value = column.searchValue ? column.searchValue(row) : getNestedValue(row, column.key);
                return String(value ?? "").toLowerCase().includes(query);
            })
        );
        // eslint-disable-next-line react-hooks/exhaustive-deps
    }, [sortedData, searchValue, searchable, visibleColumns.map(colId).join("|")]);
                    ...
                    <div>
                        ...
                        {toolbar.description && (
                            <p className="mt-0.5 text-xs text-[var(--muted)]">
                                {typeof toolbar.description === "function"
                                    ? toolbar.description(filteredData.length)
                                    : toolbar.description}
                            </p>
                        )}
                    </div>
